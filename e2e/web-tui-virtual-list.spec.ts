...
test("100k-row virtual List stays bounded across keyboard, pointer, and scroll", async ({ page }) => {
  ...
  const status = section.getByRole("status", { name: "Virtual list status" });
  ...
  await expect(status).toHaveAttribute("data-mounted", "11");
  ...
  await expect(section.getByRole("option", { name: /000010\s+src\/file-000010\.ts/ }))
    .toBeFocused();
  await expect(status).toHaveAttribute("data-scroll-y", "9");
  await page.keyboard.press("PageUp");
  await expect.poll(async () => (await readCellProbe(surface)).focusedId)
    .toBe("virtual-file-0");
  await expect(status).toHaveAttribute("data-scroll-y", "0");

  for (let index = 0; index < 10; index += 1) {
    await page.keyboard.press("ArrowDown");
  }
  await expect(surface).toHaveAttribute("data-cell-focused", "virtual-file-10");
  await expect(status).toHaveAttribute("data-scroll-y", "2");
  await expect(section.getByRole("option", { name: /000011\s+src\/file-000011\.ts/ }))
    .toHaveAttribute("aria-posinset", "11");
  await expect(section.getByRole("option", { name: /000011\s+src\/file-000011\.ts/ }))
    .toBeFocused();
  expect(Number(await status.getAttribute("data-mounted"))).toBeLessThanOrEqual(13);
  ...
  await expect(status).toHaveText("file-5 opened");
  ...
  await page.mouse.up();
  await expect(status).toHaveAttribute("data-scroll-y", "5");
  expect(Number(await status.getAttribute("data-mounted"))).toBeLessThanOrEqual(13);

  await page.mouse.wheel(0, 100);
  await expect(status).toHaveAttribute("data-scroll-y", "6");
