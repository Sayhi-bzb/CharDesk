...
[Widget 状态规范](widgets) · [消费接口](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-ui/README.md)
...
统一外观覆盖 Button、Checkbox、Select、Toggle、RadioGroup/RadioItem、Slider/RangeSlider、List、Menu、Tree、Tabs、Grid、TextInput/TextArea。能力表区分整控件反色、thumb 字符强调与编辑区域；编辑器在同一外观出口消费实际焦点，规则由 [Widget 规范](widgets)拥有，painter 不再覆盖边框颜色。`collection-chrome.ts` 统一选择标记、树展开列和内容缩进。`grid-navigation.ts` 拥有坐标导航与入口选择，FocusManager 保存 Grid 入口 ID。TestPilot 代表活跃宿主，Browser adapter 提供真实宿主焦点，两者独立投影输入来源。底座仍属于同一个包，内部控制器不是新的公开插件 API。
