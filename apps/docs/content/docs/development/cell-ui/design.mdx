...
2. **Every Cell is Unicode**：前景数据是 `Cell.text`，不要求全部通过字体渲染；已登记 Cell graphics 按 Cell 几何确定性绘制，其他 grapheme 走字体。复制、保存与字符快照始终保留原 Unicode；背景、clip、owner 与 hit 是 Cell metadata。登记范围与绘制规则由[共享渲染契约](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/rendering/README.md#cell-graphics)拥有。
