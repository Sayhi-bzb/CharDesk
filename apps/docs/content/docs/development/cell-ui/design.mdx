...
[返回事实白板](overview) · [Macintosh 标准](macintosh) · [Widget 规范](widgets)
...
视觉与交互判断以 [Classic Macintosh → Cell UI](macintosh) 为标准；OpenTUI 只提供终端构图与工程参考。

## 状态与底座权威

[Widget 规范](widgets)拥有高亮、选择、按压、确认、编辑与连续操控规则；[Cell Primitives 底座](primitives)拥有共享控制器、组件行为、反馈与外观的依赖边界。页面只组合内容、行为与 Cell 布局，不重新实现这些机制。
