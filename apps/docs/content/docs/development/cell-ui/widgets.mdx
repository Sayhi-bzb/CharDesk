...
- TextInput、ComboboxInput 与 TextArea 的编辑活动由 Runtime 比对 `focusedId === activeFocusId` 后投影为 `focusActive`，不依赖输入来源、hover 或正文变化。TextInput 默认 `elevated`，活动时整行 highlight；`plain` 与 ComboboxInput 保留背景。两种 TextInput variant 与 ComboboxInput 都用下划线表示活动选区，并用反色 Cell Cursor 表示插入点。TextArea 活动时 highlight 覆盖文字、空白与边框。未激活的单行输入不显示预先存在的选区。Surface 空白只接管浏览器焦点并保留导航锚点，不激活编辑器。readOnly 可聚焦，disabled 无活动强调。
...
- [Block 外观契约](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-ui/README.md#theme-consumption)分别定义背景与边框；后代状态消费最近的 Block substrate。背景不进入字符、语义或 Range Copy；边框会进入。
- Select 与 Combobox 的 `variant` 共同决定 Trigger/Input 和 Content 背景：默认 `elevated` 使用 `elevatedSurfaceStyle`，`plain` 透明。Content 的 `frame` 独立控制下拉层边框。TextInput 的同名 `variant` 只控制自身行，默认 `elevated`；局部背景覆盖主题背景。ComboboxInput 与 plain TextInput 活动时保留背景，SelectTrigger 的 focus 与 press 沿用底色反色规则，disabled 保留底色并使用禁用文字色。
...
- 统一控件的按压与高亮最多反色一次；确认阶段独占颜色，相对进入时的有效颜色对执行反色／恢复，disabled 拒绝全部临时强调。default Button 保留 primary 基底，elevated 使用 `elevatedSurfaceStyle` 与普通文字色，outline 保留字符边框，ghost 静止时透明。
...
| Dialog / Title / Description / Footer | 复用 Overlay，默认居中、36 Cell 宽、elevated 背景及边框；Title/Description 保留字符，Footer 只排列按钮 | 条件挂载；modal/outside-click 默认开启；初始焦点、模态 Tab 循环、Escape 与恢复焦点由共享底座拥有 |
...
| Combobox | 单行填充 Input＋`▾/▴`，Content 复用 Select 的浮层、scroll 与 `✓`；active item 临时反色 | 关闭时点击输入行任意位置展开；展开后文字区保留编辑，箭头切换关闭。DOM focus 始终留在 Input，`aria-activedescendant` 指向 active item；本地稳定子串过滤，Enter/click 提交，Escape/Tab/outside 恢复已选 label；IME 只在 composition commit 后过滤 |
...
| TextInput / ComboboxInput | 固定 1 Cell 高、无边框；TextInput 默认 elevated，实际焦点驱动整行反色，plain TextInput 与 ComboboxInput 保留背景；三者都以选区下划线及反色 Cell Cursor 表示编辑，并保留 selection、composition | 真实 textarea；textbox/combobox value/readOnly/disabled；style 仅允许宽度与 flex 约束 |
