...
| Dialog / Title / Description / Footer | 复用 Overlay，默认居中、36 Cell 宽；`surface` 使用 elevated surface token，`ghost` 使用当前层底色，弹窗均不透明；`border="none" | "square" | "rounded"` 独立控制边框，省略时沿用主题形状；Title/Description 保留字符，Footer 只排列按钮 | 条件挂载；modal/outside-click 默认开启；初始焦点、模态 Tab 循环、Escape 与恢复焦点由共享底座拥有 |
