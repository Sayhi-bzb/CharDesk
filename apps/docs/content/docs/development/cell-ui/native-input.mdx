---
title: Cell UI Native Input
description: macOS input-source verification through real browser textareas.
---

# Cell UI 原生输入验证

本页登记 macOS 输入系统到真实浏览器 textarea 的证据。交付状态见 [Roadmap](/development/cell-ui/status)；编辑状态与合成事件由 [`text.test.tsx`](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-ui/src/text.test.tsx) 和 [`browser.dom.test.tsx`](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-ui/src/browser.dom.test.tsx) 负责。
