...
[返回事实白板](/development/cell-ui/overview) · [UI 哲学](/development/cell-ui/design) · [Widget 规范](/development/cell-ui/widgets)
...
- 当前操作目标与持久选择分开表达；鼠标和键盘消费同一种操作强调，确认反馈不叠加反色。具体优先级、组件迁移范围与字符标记由 [Widget 状态规范](/development/cell-ui/widgets#状态与主题)拥有。
- Button variant 在静止状态可区分：solid 使用高对比底色，surface 使用抬升底色，outline 使用字符边框，ghost 沿用当前层底色。Select、Combobox 与 TextInput 共享 `surface | ghost` recipe。
