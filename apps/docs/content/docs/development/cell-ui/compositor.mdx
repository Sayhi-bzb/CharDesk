---
title: Cell UI Compositor
description: SceneGeometry, clipping, layering, and paint contracts.
---

# SceneGeometry / Compositor 契约

[返回事实白板](/development/cell-ui/overview) · [SemanticSnapshot](/development/cell-ui/semantics)
...
逻辑 geometry、source 与 frame 类型及通用 Rect 运算由 [`@chardesk/cell-core`](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-core/README.md) 持有；跨后端的 Cell metrics、frame、cursor paint 契约与字符检查出口由 [`@chardesk/rendering`](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/rendering/README.md) 根入口持有；Widget、Layout、Scene 与 Semantic 类型由 [`@chardesk/cell-ui`](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-ui/src/types.ts) 持有。本页不复制类型声明。
...
- ScrollArea 与 Select／Combobox 弹层的自动宽度子项按扣除可见 rail 后的 viewport 布局。Runtime 在布局与 Scene 间收敛轨道占位；占位独立于用户 padding，只有溢出时存在。仅改变 offset 不重新布局。
...
- 浏览器 Surface 的格宽、格高和基线由同步 Cell metrics 决定；`CellSurface`、CharDesk Canvas、[rendering](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/rendering/README.md#fixed-cell-grids-and-font-measurement) 与 Viewer 的产品默认值统一为 `9×20 / 15px / baseline 15`，显式 metrics 优先。所有绘制与输入坐标共享结果；字体加载和切换不改变 Cell 占位、viewport 或已挂载布局，只触发重绘与审计。旧 `9×19` 持久化 viewport 只在 schema 升级时按高度比迁移一次。
...
[`TestPilot`](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-ui/src/testing.ts) 从提交后的 frame 执行逻辑输入和查询；[`CellProbeSnapshot`](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-ui/src/probe.ts) 是 Headless 与 Browser 共用的字符、owner、style、hit、clip、focus 和 invalidation 快照；v4 额外分离 base/overlay viewport 与每个可见 portal 的字符快照。截图只负责字体、颜色、DPR 与像素 presentation。

[`check:cell-architecture`](https://github.com/Sayhi-bzb/CharDesk/blob/main/scripts/quality/check-cell-architecture.mjs)
阻止 Core 获得运行时依赖、两个 Frame adapter 互相依赖、rendering 根入口反向
依赖 Canvas presenter，以及已移除的 Canvas-owned Cell 契约回流。

## 证据

[Runtime tests](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-ui/src/runtime.test.tsx) · [Performance tests](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-ui/src/performance.test.tsx) · [Probe tests](https://github.com/Sayhi-bzb/CharDesk/blob/main/packages/cell-ui/src/probe.test.tsx)
