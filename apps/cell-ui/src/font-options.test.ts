...
describe("Cell UI gallery font profiles", () => {
  it("consumes profiles owned by the font packages", () => {
    expect(galleryFontOptions["fusion-mono"].profile).toBe(FUSION_FONT_PROFILE);
    expect(galleryFontOptions["xiaolai-mono"].profile).toBe(XIAOLAI_FONT_PROFILE);
  });
