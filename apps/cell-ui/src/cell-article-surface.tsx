...
export function CellArticleSurface({ label, probeId, content, anchorIds, focusedId, onCommand }: ArticleSurfaceProps) {
  const hostRef = useRef<HTMLDivElement>(null);
  const [width, setWidth] = useState(72);
  useLayoutEffect(() => {
    const host = hostRef.current;
    if (!host) return;
    const update = () => setWidth(Math.max(1,
  ...
  const measured = useMemo(() => {
    ...
    try {
      ...
      return {
        ...
        anchors: Object.fromEntries(anchorIds.map((id) => [id, entries.get(`${probeId}-${id}`)?.rect.y ?? 0])) as Record<string, number>,
      };
    } finally {
      layout.dispose();
    }
  }, [width, content, anchorIds, probeId]);
