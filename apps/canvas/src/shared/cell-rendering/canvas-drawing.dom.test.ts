...
import {
  ...
  getCellViewportRect as gridCellRect,
  resolveCharDeskFontRoute as resolveRenderFontRoute,
...
const isWideCell = (grapheme: string) => getCellOccupancy(grapheme) === 2;
const getRenderFontFamily = (route: "text" | "emoji") =>
  MAPLE_FONT_PROFILE.families[route];
const loadRenderFonts = (samples: Parameters<typeof loadCharDeskCanvasFonts>[0]) =>
  loadCharDeskCanvasFonts(samples, { fontProfile: DEFAULT_CANVAS_FONT_PROFILE });

describe("metrics", () => {
  it("uses the app-owned canvas metrics", () => {
    expect(DEFAULT_CANVAS_CELL_METRICS).toMatchObject({
      cellWidth: 9,
      cellHeight: 20,
      baseline: 15,
      fontSize: 15,
    });
    expect(Object.isFrozen(DEFAULT_CANVAS_CELL_METRICS)).toBe(true);
  });
  ...
  describe("viewport conversion", () => {
    it("converts grid and screen coordinates through shared metrics", () => {
      ...
      expect(screen).toEqual({
        x: 10 + 3 * DEFAULT_CANVAS_CELL_METRICS.cellWidth * 2,
        y: 20 + 4 * DEFAULT_CANVAS_CELL_METRICS.cellHeight * 2,
      });
    ...
    it("returns cell rects from the same viewport model", () => {
      ...
      expect(rect).toEqual({
        x: 5 + 2 * DEFAULT_CANVAS_CELL_METRICS.cellWidth * 1.5,
        y: 7 + 3 * DEFAULT_CANVAS_CELL_METRICS.cellHeight * 1.5,
        width: DEFAULT_CANVAS_CELL_METRICS.cellWidth * 1.5,
        height: DEFAULT_CANVAS_CELL_METRICS.cellHeight * 1.5,
      });
    });
  });
