...
- Button、Checkbox、Select、Toggle、RadioItem、List、Menu、Tree、Tabs 共用反色高亮：鼠标与 keyboard/AT 采用最近输入来源，逻辑 focus 独立保留；pointer 移出撤销高亮，键盘恢复同一套高亮。不增加焦点 bold 或 `▶`。Slider/RangeSlider 同样投影输入来源，但仅强调目标 thumb。
- Checkbox checked/mixed、Select selected、Toggle pressed、Radio checked、List/Tree selected 使用独立字符标记；Tabs 使用当前页下划线。不提供持久选择背景；Grid 与编辑器保留原状态配方。
...
- pointer down 定位焦点；Button、SelectTrigger、SelectItem、Checkbox、Toggle、RadioItem、ListItem、MenuItem、TreeItem、Tab 移出时撤去按压、移回时恢复，pointer up 仍命中原目标才激活。drag winner、cancel、capture loss 或 blur 永久取消本轮 press。
