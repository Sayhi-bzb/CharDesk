...
describe("Web TUI gallery font loading", () => {
  afterEach(() => {
    vi.restoreAllMocks();
    localStorage.removeItem("chardesk-web-tui-font");
  ...
  it("selects and commits Fusion only after both Latin and CJK samples load at the Canvas size", async () => {
    ...
    expect(localStorage.getItem("chardesk-web-tui-font")).toBe("fusion-mono");
  ...
  it("keeps Maple committed and leaves the preference unchanged when loading fails", async () => {
    ...
    expect(localStorage.getItem("chardesk-web-tui-font")).toBeNull();
  ...
  it("ignores an invalid saved font", () => {
    vi.spyOn(HTMLCanvasElement.prototype, "getContext").mockReturnValue(null);
    localStorage.setItem("chardesk-web-tui-font", "unknown");
    const { container } = render(
      <GalleryAppearance><GalleryFontSelect /></GalleryAppearance>
    );
