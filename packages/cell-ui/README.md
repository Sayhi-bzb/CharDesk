...
`CellSurface.fontProfile` selects text faces. The Surface derives one effective Profile whose `cell-glyph` capability always uses Core JuliaMono Regular for Box Drawing and Block Elements (`U+2500–U+259F`). Loading, Canvas painting (including the caret), audit and Probe share that Profile; `requestedFontRoutes["cell-glyph"]` exposes it. Changing the input Profile forces a full repaint without changing protocol Cell widths. The main Canvas Host uses the same Core routing helper; the generic renderer retains its display-based default. This routing does not certify seamless glyphs at arbitrary sizes: at 15px, JuliaMono `█` has approximately 17.625px of ink within the default 20px row.
