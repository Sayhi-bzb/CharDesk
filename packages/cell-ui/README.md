...
`inline-control-chrome.ts` owns inline spacing recipes. Structural guard, indicator, and gap geometry remains distinct from default content insets: Checkbox, RadioItem, Toggle, Select, and Combobox use owner-aware blank guard Cells, while Button `size` resolves to `0/1/2` content Cells and TextInput retains one. Consumer Button/TextInput padding replaces the default content inset; consumer padding on guarded controls is added inside their chrome. Checkbox renders ` [ ] Label `, RadioItem ` (●) Label `, and Toggle ` ● Bold `. `CellUiTheme.toggleOffIndicator` and `toggleOnIndicator` supply Toggle glyphs; persistent state remains semantic and independent of transient emphasis.
