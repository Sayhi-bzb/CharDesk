...
Gallery accepts feedback once on `GalleryAppearance` and distributes it to every `GallerySurface`. `widget-capabilities.ts` owns feedback areas/capabilities; `visual.ts` owns state precedence and primary/ordinary/thumb recipes. Painter consumes their resolved style and glyph. Primary keyboard focus preserves primary colors and adds bold.
