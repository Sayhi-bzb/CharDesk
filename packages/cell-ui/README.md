...
`inline-control-chrome.ts` owns inline spacing recipes. Structural guard, indicator, and gap geometry remains distinct from default content insets: Checkbox, RadioItem, Toggle, Select, and Combobox use owner-aware blank guard Cells, while Button and TextInput default to one content Cell on each side. Consumer Button/TextInput horizontal padding replaces the default content inset; consumer padding on guarded controls is added inside their chrome. Checkbox renders ` [ ] Label `, RadioItem ` (●) Label `, and Toggle ` ● Bold `. `CellUiTheme.toggleOffIndicator` and `toggleOnIndicator` supply Toggle glyphs; persistent state remains semantic and independent of transient emphasis.
