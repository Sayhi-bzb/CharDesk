...
Keyboard and semantic focus use the same inverse control treatment as pointer highlight, without additive bold. Pointer input retains logical focus but presents only transient component hover; moving the pointer away clears it. Editors keep their own surface, caret, and selection styling. Headless rendering remains modality-neutral unless the host supplies focus options.
...
`Toggle`, `Progress`, `Separator`, `RadioGroup` and `RadioItem` consume the same theme and capability resolver. Progress accepts a clamped numeric `value` or `null` for indeterminate projection, supports `solid` and `outline` variants, and can show the calculated percentage with `number`. Both variants consume `progressGlyphs`; outline brackets and an optional percentage remain inside the declared Cell width. Headless hosts supply `animationTimeMs`, while `CellSurface` drives it automatically unless reduced motion is requested. Their [Gallery pages](../../apps/cell-ui/src/component-catalog.tsx) own executable usage examples; the [Widget contract](../../apps/docs/content/docs/development/cell-ui/widgets.mdx) owns behavior. `useCellRadioState(items, { value?, defaultValue?, disabled?, onValueChange? })` is exported from `/browser`; its `dispatch` accepts focus, activate and immediate `select-radio` navigation commands. Item IDs target commands; item values determine selection. Group `disabled` must also be passed to the state adapter when dispatching commands directly.

`Badge` shows a short status with one content Cell on either side. `tone` selects a paired theme color (`neutral`, `info`, `success`, `warning`, or `error`); `interactive` opts into Button-like focus and `activate` behavior and requires an `id`. Static Badges do not act as live regions. [Gallery usage](../../apps/cell-ui/src/component-catalog.tsx) · [Behavior contract](../../apps/docs/content/docs/development/cell-ui/widgets.mdx).

`Spinner` uses the same headless `animationTimeMs` clock as indeterminate Progress; `CellUiTheme.spinnerGlyphs` supplies its wheel and dots sequences. Its [Widget contract](../../apps/docs/content/docs/development/cell-ui/widgets.mdx) owns projection and accessibility behavior.

`Tooltip` binds supplementary one-line text to a focusable Cell control by `targetId`. Headless hosts explicitly supply `tooltipTargetId` to `render`; `CellSurface` handles delayed mouse hover and immediate keyboard focus. The [Widget contract](../../apps/docs/content/docs/development/cell-ui/widgets.mdx) owns its placement and accessibility behavior.
...
Surface-owning components use independent background and border props. Box, ScrollArea, TextArea, and Overlay use `variant` with `frame`; `variant="ghost"` is transparent and inherits the nearest rendered substrate, while `"surface"` fills with `elevatedSurfaceStyle`. `frame="bordered"` reserves one Cell per edge for Unicode chrome. Their defaults are ghost/none for Box, ScrollArea, and TextArea; surface/none for Overlay. Dialog and Tooltip instead use opaque `variant="surface" | "ghost"` with `border="none" | "square" | "rounded"`; omitting `border` keeps the frame and inherits `CellUiTheme.borderShape`. Other framed components use `borderShape` for the same square or rounded glyphs (`╭╮╰╯`). Shape is paint-only and rounded corners do not clip the rectangular background. `CellLayoutStyle` owns geometry only. Public Props expose `label` only on semantic widgets; Select and Combobox labels belong on their Trigger or Input, not the outer layout descriptor. TextInput, TextArea, and ComboboxInput do not accept Cell children.

`render(..., { hoveredId })` supplies optional transient hover; omission clears it. `WidgetNode.hovered` affects paint only, not layout or semantic nodes. Button, Checkbox, Select, Toggle, RadioItem, List, Menu, Tree and Grid use inverse highlight; Tabs default to `underline` and retain the previous inverse selection as `variant="solid"` ([behavior contract](../../apps/docs/content/docs/development/cell-ui/widgets.mdx#状态与主题)). `hoveredItemStyle` / `--cell-hover` remains the other components’ hover recipe. Button supports `solid`, `surface`, `outline`, and `ghost`; Select, Combobox, and TextInput share the `surface` and `ghost` subset. Solid Buttons use `buttonSolidStyle`; outline controls retain bracket chrome and ghost controls remain transparent. Progress independently supports `solid` and `outline`. Disabled solid Buttons return to `surfaceStyle` before `disabledStyle` is applied. `GestureManager.manipulatingIds` owns pointer-captured continuous manipulation and `render(..., { manipulatingIds })` projects it as paint-only `WidgetNode.manipulating`; it starts on pointer down rather than after the drag threshold and clears on release, cancellation, capture loss, blur, or candidate invalidation. Slider emphasizes only its thumb from `sliderThumb` (`┃`) to `sliderEmphasizedThumb` (`█`) while hovered, keyboard-focused or manipulating; RangeSlider uses the same recipe per thumb. `CellSurface` derives hover and the browser pointer from Scene hits and modal scope, updates only when the resolved target changes, and rechecks stationary pointers on frame/scroll/resize changes. Hover is mouse-only; down/drag suspends it, up restores it, and leave/cancel/capture loss/window blur clears it. Manipulation bridges that suspension without pretending the pointer is still hovering. Select/Menu item hover emits provisional focus, never selection or execution; other hover and manipulation projections do not emit business commands. The browser pointer is `pointer` for actionable items and `default` elsewhere, including editor content; editing position is expressed only by the Canvas Cursor.
...
`primitive-appearance.ts` owns control and collection inverse composition and Slider/RangeSlider thumb emphasis; `visual.ts` resolves editor activity, surface substrate, and final border styles; `resolveCellTextStyle(base, state, theme)` adds text selection and composition. Painter supplies geometry, not color policy. `elevatedSurfaceStyle` and `borderStyle` supply surface and frame recipes; descendants inherit the nearest effective surface background. The [Widget contract](../../apps/docs/content/docs/development/cell-ui/widgets.mdx#状态与主题) owns single-line control backgrounds and state behavior. `cursorStyle` / `rangeStyle` configure browser overlays. `background` / `foreground` supply the Canvas palette unless `CellSurface.palette` is explicitly provided. The private experimental API does not retain compatibility aliases.
...
`inline-control-chrome.ts` owns inline spacing recipes. Structural guard, indicator, and gap geometry remains distinct from default content insets: Checkbox, RadioItem, Toggle, Select, and Combobox use owner-aware blank guard Cells, while Button, Badge, and TextInput default to one content Cell on each side. Consumer Button/Badge/TextInput horizontal padding replaces the default content inset; consumer padding on guarded controls is added inside their chrome. Checkbox renders ` [ ] Label `, RadioItem ` (●) Label `, and Toggle ` ● Bold `. `CellUiTheme.toggleOffIndicator` and `toggleOnIndicator` supply Toggle glyphs; persistent state remains semantic and independent of transient emphasis.

List/Tree/Grid reserve a fixed selection column using `collectionSelectedIndicator` (default `✓`); Tree disclosure uses a separate column. Tabs spacing and variant geometry are defined by the [Widget contract](../../apps/docs/content/docs/development/cell-ui/widgets.mdx#primitive-快照). These controls activate immediately without confirmation flashing. Menu is different: Surface defers `activate` and therefore `useCellMenuState.onAction` until confirmation completes; `0` is immediate. Cancellation discards the pending action. Callers close overlays or navigate in `onAction`; the engine does not close arbitrary ancestors. Direct adapter dispatch has no presentation lifecycle. See the [feedback lifecycle](../../apps/docs/content/docs/development/cell-ui/primitives.mdx#生命周期).
...
The `/browser` entry exports `readCellCssTheme(element)` and `useCellCssTheme(ref, revision)`, returning `{ theme, palette, recipe }`. The hook reads after mount and whenever the explicit string/number revision changes. Apply CSS changes before its layout effect; external stylesheet edits require a revision bump. No DOM mutation observer, per-frame CSS read, or DOM-per-Cell is created. Missing/invalid fields use `DEFAULT_CELL_UI_THEME` and component recipe defaults; development builds report the token name.

Supported color tokens (prefix each with `--cell-`): `background`, `foreground`, `surface`, `surface-elevated`, `button-solid`, `button-solid-foreground`, `badge-{neutral,info,success,warning,error}` and each badge tone's `-foreground` pair, `hover`, `highlight`, `highlight-foreground`, `muted-foreground`, `disabled-foreground`, `border`, `selection`, `selection-foreground`, `range-surface`, `range-border`, `cursor`, `cursor-foreground`, `scrollbar-thumb`, `scrollbar-track`. Browser CSS resolves inherited values and aliases; the adapter resolves colors before Canvas consumes them. `surface-elevated` is a visual depth cue only and never enters Cell text or semantics. `highlight` feeds the legacy focus/selection recipes; Button derives inverse colors from its base pair. `--cell-accent` is consumed directly by host CSS for links and focus outlines, not by the headless engine.

`--cell-default-control-variant` accepts `surface | ghost` and maps to `CellUiRecipe.defaultControlVariant`. It supplies omitted variants for Button, Select, Combobox, and TextInput in one Surface; an explicit component `variant` wins. Box, ScrollArea, TextArea, Overlay, and Dialog keep their built-in defaults unless explicitly configured. Without the token or a headless `recipe`, controls also retain their built-in defaults. Checkbox, RadioItem, and Toggle do not expose this variant axis.

```tsx
const container = useRef<HTMLDivElement>(null);
const { theme, palette, recipe } = useCellCssTheme(container, themeRevision);
return <div ref={container}>
  <CellSurface theme={theme} palette={palette} recipe={recipe} {...surfaceProps} />
</div>;
```
...
const frame = runtime.render(
  <Root id="root">
    ...
    <ScrollArea frame="bordered" style={{ height: 6 }}>
      <List>
...
`Root`, `Box`, `Overlay`, `Text`, `Button`, `Checkbox`, `Slider`, `RangeSlider`, `Select`, `Combobox`, collection widgets, and text editors are descriptors consumed by `CellUiRuntime`; they are not DOM components. `Button` is a one-row actionable descriptor with one default content Cell on each side; `style.paddingLeft` and `style.paddingRight` override those insets without changing vertical geometry; its `surface` variant uses `elevatedSurfaceStyle` with ordinary foreground color, and pointer, keyboard, and semantic input emit the same `activate` command. `Checkbox` accepts `false`, `true`, or `"indeterminate"`; `nextCellCheckboxState()` maps mixed to checked before entering the binary toggle cycle. `Slider` owns a one-row `━/─/┃` track and emits controlled `set-value` commands from stepped keyboard input, precise track taps, and drag. `RangeSlider` owns the same track with exactly two direct `RangeSliderThumb` children; each thumb has its own required `id`, `label`, controlled `value`, focus, and `set-value` target, while the interval between them uses `━`. Track taps choose the nearest thumb, thumbs cannot cross, and the selected interval is not draggable. Visible labels and values remain ordinary Cell composition. `Select` composes `SelectTrigger`, `SelectContent`, and `SelectItem`; its Content portals beside the Trigger, flips above when required by the Cell viewport, and keeps provisional focus separate from committed selection. Select and Combobox use the shared inline chrome recipe for their leading guard and trailing gap, indicator, and guard. `Combobox` composes one `ComboboxInput` and optional `ComboboxContent` with `ComboboxItem` candidates; its input remains the focus owner while active candidate and committed selection stay independent. `TextInput` and `ComboboxInput` are fixed one-Cell-high, borderless surfaces; their public `style` accepts only width constraints and flex growth/shrinkage, and descriptor normalization discards multiline geometry fields at runtime. `TextArea` owns multiline geometry, surface appearance, padding, and rails. The collection set includes `List`, `Menu`, `Tree`, `Tabs`, and `Grid` with their item, row, and panel descriptors. Containers accept integer Cell layout through `style`; all four padding edges may be overridden independently. Explicit `id` values are globally stable; keyed children receive parent-scoped stable ids; unkeyed children follow React position identity.
...
<ScrollArea id="files" frame="bordered" scrollY={files.scrollY} style={{ height: 11 }}>
