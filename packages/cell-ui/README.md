...
`CellSurface` separates retained logical `focusedId` from browser focus ownership. It passes actual Surface/window activity through the runtime's existing `focusVisible` option; focus backgrounds, focus bold, the Canvas Cursor, and `data-cell-focus-visible` follow that same state. Blur preserves selection and editing state. Internal DOM focus transfers retain activity; external/null-target blur and window changes recheck ownership before restoring focus. Headless defaults are unchanged.
