...
Block-owning components use independent `variant` and `frame` props. `variant="plain"` is transparent and `"elevated"` fills with `elevatedSurfaceStyle`; `frame="bordered"` reserves one Cell per edge for Unicode chrome. Defaults are plain/none for Box, ScrollArea, and TextArea; elevated/none for Overlay; elevated/bordered for Dialog. `borderShape` accepts `"square"` or `"rounded"` (`╭╮╰╯`) and otherwise inherits `CellUiTheme.borderShape`. Shape is paint-only and rounded corners do not clip the rectangular background. `CellLayoutStyle` owns geometry only.
