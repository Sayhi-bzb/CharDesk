...
describe("CellUiRuntime", () => {
  ...
  it("allows hosts to explicitly suppress focus styling without clearing logical focus", () => {
    ...
    expect(frame.buffer.get(3, 0)?.text).toBe("O");
  ...
  it.each([
    ...
    expect(frame.buffer.get(0, 1)).toMatchObject({ text: "│", ownerId: "boundary" });
    expect(frame.buffer.get(9, 1)).toMatchObject({ text: "│", ownerId: "boundary" });
    expect(frame.buffer.toText({ trimEnd: true }).split("\n")[2]).toBe("└────────┘");
    runtime.dispose();
  });
  ...
  it("portals an Overlay to a root layer while preserving its event parent", () => {
    ...
    expect(hitTest(frame.scene, { x: 7, y: 3 })[0]).toBe("command-two");
