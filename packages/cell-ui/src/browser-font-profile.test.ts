...
import { getCharDeskCanvasFont, resolveCharDeskCanvasFontFace, DEFAULT_CHARDESK_CANVAS_METRICS } from "@chardesk/rendering/canvas";
import { createCellUiFontProfile } from "./browser-font-profile.js";

describe("Cell UI Core glyph routing", () => {
  it("keeps the document Maple face while Surface borders use regular Core", () => {
    ...
    for (const grapheme of ["─", "│", "╭", "└", "█", "▀", "▄", "▐"]) {
      ...
      expect(surfaceFace).toMatchObject({ capability: "cell-glyph", family: "'JuliaMono'", weightPolicy: "regular" });
      expect(getCharDeskCanvasFont(DEFAULT_CHARDESK_CANVAS_METRICS, 1, {
        bold: true, fontFamily: surfaceFace.family, weightPolicy: surfaceFace.weightPolicy,
      })).toBe("15px 'JuliaMono'");
    }
  ...
  it("enforces the Core route even with a custom classifier while retaining other routes", () => {
    ...
    expect(surface.resolveCapability("╭")).toBe("cell-glyph");
