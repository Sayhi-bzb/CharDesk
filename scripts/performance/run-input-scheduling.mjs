  ...
  const ping = () => {
    ...
    request.on('error', () => {
      if (Date.now() - startedAt > timeoutMs) {
        reject(new Error('Input scheduling preview timed out'));
      } else {
        setTimeout(ping, 250);
      }
    });
...
try {
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on('exit', (code, signal) => code === 0
      ? resolve()
      : reject(new Error(`Input scheduling benchmark failed with ${signal ?? `code ${code}`}`)));
  });
  console.log(`Canvas input scheduling report: ${path.join(reportDir, 'report.md')}`);
} finally {
  stop();
}
