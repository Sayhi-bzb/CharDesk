...
const managedKeyDownEvent = (init: KeyboardEventInit, preventDefault = vi.fn()) => ({
...
const getEditorModel = () => {
  ...
  return {
    ...
    writeTextString: commands.text.write,
    newlineText: commands.text.newline,
    indentText: commands.text.indent,
    moveTextCursor: commands.text.moveCursor,
...
describe("useManagedCanvasInput", () => {
  ...
  it("keeps consecutive half-width input, including Space, on one advancing grid flow", () => {
    ...
    expect(
      [...getEditorModel().contentSurface.reader.materialize().values()].map(({ char }) => char)
    ).toEqual(["A", " ", "B"]);
  });
        ...
        useManagedCanvasInput({
          model: {
            ...
            interaction: {
              ...
              staticGrid: {
                mode: "text-edit",
                session: createStaticGridInputSession({
                  origin: { x: 1, y: 0 },
                }),
              },
            },
